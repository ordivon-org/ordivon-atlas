#!/usr/bin/env python3
"""Frozen one-shot actor runner for C3 E1.
...
PRIVATE = BASE / "e1-efficacy-private"
RESULTS = BASE / "e1-efficacy-results"
MANIFEST_PATH = BASE / "E1-EFFICACY-PUBLIC-MANIFEST.json"
E1_PATH = BASE / "e1_micro_world.py"
HARNESS_ROOT = Path("/root/projects/ordivon-harness")

EXPECTED = {
    "manifest_sha256": "2ba6f0e4acdb82dcbf6cfacddcc95bcd6c13645ccd9e85e90a6bf12e876e48ad",
...
def load_manifest() -> dict[str, Any]:
    if sha256_file(MANIFEST_PATH) != EXPECTED["manifest_sha256"]:
        raise RuntimeError("E1 public manifest digest mismatch")
    value = json.loads(MANIFEST_PATH.read_text())
    if value.get("n_instances") != 8 or value.get("use_count") != 4 or value.get("suppress_count") != 4:
        raise RuntimeError("E1 public manifest cardinality mismatch")
    return value
...
def preflight() -> dict[str, Any]:
    ...
    return {
        "status": "E1_ACTOR_PREFLIGHT_PASS",
        "instances": 8,
        "use": 4,
        "suppress": 4,
        "model": s.model,
...
def run_one(index: int) -> dict[str, Any]:
    ...
    s = settings()
    adapter = DeepSeekTurnAdapter(s, completion_contract=completion_schema)
    request = AgentTurnRequest(
        harness_run_id=f"harness-run:c3-e1:{index:02d}",
        turn_id=f"turn:c3-e1:{index:02d}:1",
        sequence=1,
        assignment_id=f"assignment:c3-e1:{index:02d}",
        context_digest=canonical_digest({
            "e1": "efficacy-context-v1",
            "instanceDigest": row["instance_digest"],
            "actorUserDigest": row["actor_user_digest"],
        }),
        tool_catalog_digest=canonical_digest({
            "e1": "no-runtime-tools-v1",
    ...
    record: dict[str, Any] = {
        "schema": "ordivon.c3.e1-actor-result.v1",
        "index": index,
        "arm": arm,
